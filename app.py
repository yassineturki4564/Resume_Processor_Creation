...
def get_download_link(filename, text):
    ...
    return href
def apply_category_colors(df):
    """
    Color the cells in the 'Category' column.
    """
    def color_cells(val):
        if val == 'Green':
            color = 'green'
    ...
    # Apply styling
    styled_df = df.style.applymap(color_cells, subset=['Category'])
    return styled_df

def color_dataframe(df, green_len, yellow_len):
    # 1. Add and assign category
    df['Category'] = ''
    df.loc[:green_len-1, 'Category'] = 'Green'
    df.loc[green_len:green_len+yellow_len-1, 'Category'] = 'Yellow'
    df.loc[green_len+yellow_len:, 'Category'] = 'Red'
    
    # 2. Convert list values to string
...
def main():
    ...
    if uploaded_files:
        ...
        if st.button("Process"):
            ...
            green_len = int(0.3 * len(df_simple))
            yellow_len = int(0.3 * len(df_simple))
            df_simple = color_dataframe(df_simple, green_len, yellow_len)
            styled_df = apply_category_colors(df_simple)
